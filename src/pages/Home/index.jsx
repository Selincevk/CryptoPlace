...
useEffect(() => {setDisplayCoin(allCoin)},[allCoin, CoinContext])
...
<form onSubmit={searchHandler} className="flex w-full max-w-md bg-white rounded-lg shadow-md overflow-hidden">
    <input type="text" placeholder="Search Crypto" className="flex-1 px-4 py-2 text-base border-none text-black" list="coinlist" value={input} onChange={inputHandler}/>
{/* DATALİST -- input eşlemesi ile inputa otomatik tanımlama desteği sunabiliriz */}
    <datalist id="coinlist">
{allCoin.map((item,key) => (
    <option key={key} >{item.name} </option>
))}
    </datalist>
    <button type="submit" className="bg-[#1a1a40] px-5 py-2 font-semibold hover:opacity-90">
        Search
    </button>
</form>
      </div>
